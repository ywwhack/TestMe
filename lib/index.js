...
router.get('/', (() => {
  var ref = _asyncToGenerator(function* (ctx, next) {
    ...
    const filePath = token && _session.session[token] ? `${ _configure.templatePath }/message.html` : `${ _configure.templatePath }/upload.html`;
...
router.get('/profile', (() => {
  var ref = _asyncToGenerator(function* (ctx, next) {
    ctx.body = yield _babelFs2.default.readFile(`${ _configure.templatePath }/upload.html`);
    ctx.type = "html";
  }),
      _this = undefined;
...
app.use(_body.body).use(router.routes()).use(router.allowedMethods());
